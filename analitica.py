...
if op== '🤝 Concluciones':
    ...
    st.markdown("<h3 style='text-align: justify;color:#000000; '>5) Las empresa con mayor promedio de ingresos estan el departamento de Bolívar y la mayor parte de ellos provienen del macrosector manufactura, por lo que los altos ingresos que reciben las familias tiene una dependencia directa con este sector y su avance, pues la mayoría de las familia dependen de los ingresos que reciba este sector.</h3>", unsafe_allow_html=True)
    ...
    st.markdown("<h3 style='text-align: justify;color:#000000; '>8) Si existe una relación directa entre la economía de las empresas y la economia de los hogares colombianos, sin embargo  el número de pobres e indigentes no se ve afectado por la cantidad de ingresos que poseen las empresas en los diferentes departamentos, son otros factores no tenidos en cuenta en este análisis  los que afectan el número de pobres y de indigentes. </h3>", unsafe_allow_html=True)
